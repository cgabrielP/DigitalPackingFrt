...
const ROLES = [
  { key: 'PICKER',     label: 'Picker',     desc: 'Escanea y empaca órdenes',           color: 'blue'   },
  { key: 'DELIVERY',   label: 'Delivery',   desc: 'Ve órdenes listas para despacho',    color: 'green'  },
  { key: 'SUPERVISOR', label: 'Supervisor', desc: 'Ve todo, puede empacar y reasignar', color: 'purple' },
  { key: 'ADMIN',      label: 'Admin',      desc: 'Acceso completo al sistema',         color: 'amber'  },
]
...
const Alert = ({ type, msg, onClose }) => (
  <div className={`ap-alert ap-alert--${type}`}>
    ...
    {onClose && <button className="ap-alert-close" onClick={onClose}>×</button>}
  </div>
)
    ...
    <div className="ap-modal-overlay" onClick={onClose}>
      <div className="ap-modal" onClick={e => e.stopPropagation()}>
        ...
        <div className="ap-field">
          ...
          <input className="ap-input" placeholder="Nombre completo"
            value={form.name} onChange={set('name')} />
        </div>

        <div className="ap-field">
          <label className="ap-label">EMAIL</label>
          <input className="ap-input" type="email" placeholder="usuario@empresa.com"
            value={form.email} onChange={set('email')} />
        </div>

        <div className="ap-field">
          <label className="ap-label">CONTRASEÑA INICIAL</label>
          <input className="ap-input" type="password" placeholder="Mínimo 8 caracteres"
        ...
        <div className="ap-modal-footer">
          <button className="ap-btn-ghost" onClick={onClose}>Cancelar</button>
...
export default function AdminPanel() {
  ...
  const [deleting,   setDeleting]   = useState(null) // userId en proceso
  ...
  const loadUsers = async () => {
    try {
      ...
      if (res.status === 403) { navigate('/orders', { replace: true }); return }
      const data = await res.json()
  ...
  const handleCreated = (user) => {
    ...
    setShowModal(false)
    setNotice({ type: 'success', msg: `Usuario ${user.name} creado correctamente` })
      ...
      <div className="ap-page">
        ...
        <nav className="ap-nav">
          <div className="ap-nav-brand">
            <span className="ap-nav-logo">PICKING</span>
            <span className="ap-nav-dot">●</span>
            <span className="ap-nav-sub">ADMIN</span>
          </div>
          <div className="ap-nav-actions">
            <button className="ap-nav-btn" onClick={() => navigate(-1)}>
              <IconBack /> VOLVER
            </button>
            <button className="ap-nav-btn ap-nav-btn--logout" onClick={logout}>
              <IconLogout /> SALIR
            </button>
          </div>
        </nav>
        ...
        <header className="ap-header">
          ...
          <button className="ap-btn-primary ap-btn-primary--sm" onClick={() => setShowModal(true)}>
        ...
        {/* ── Notice ── */}
        {notice && (
          <Alert type={notice.type} msg={notice.msg} onClose={() => setNotice(null)} />
        )}
        ...
        <div className="ap-stats">
          {ROLES.map(r => (
            <div key={r.key} className={`ap-stat ap-stat--${r.color}`}>
              <span className="ap-stat-value">{byRole[r.key]?.length ?? 0}</span>
        ...
        {loading ? (
          ...
          <div className="ap-empty">
            ...
            <p>SIN USUARIOS</p>
            <button className="ap-btn-primary" onClick={() => setShowModal(true)}>
                    ...
                    <tr key={user.id} style={{ animationDelay: `${i * 30}ms` }}>
                      <td>
                        <div className="ap-user-cell">
                          <div className={`ap-user-avatar ap-user-avatar--${color}`}>
                      ...
                      <td>
                        <span className={`ap-badge ${user.isActive ? 'ap-badge--green' : 'ap-badge--muted'}`}>
